...
class ChessTokenizer:
    ...
    def encode_board(cls, board_state):
        ...
        assert len(
            board_tokens) == 73, f"The number of tokens encoding the board must be 71, got len(board_tokens) = {len(board_tokens)}"
        return board_tokens
    ...
    def encode_move(cls, chess_move):
        move_tokens = [cls.vocab_to_tokens[chess_move.from_square], cls.vocab_to_tokens[chess_move.to_square]]
    ...
    def decode_move(cls, move_tokens):
        ...
        return Move(cls.tokens_to_vocab[move_tokens[0]], cls.tokens_to_vocab[move_tokens[1]], promotion)
...
class ChessDataGenerator(torch.utils.data.Dataset):
    def __init__(self, pgn_file, p_sample=1., trials_limit=100, mode='train'):
    ...
    def next_game_to_raw_data(self):
        ...
        for move in enumerate(self.current_game.mainline_moves()):
            ...
            transitions.append(Transition(fen_to_board_state(board.fen()),
                                          chess_move))
    ...
    def extend_data(self, idx):
        num_trials = 0
        while idx not in self.data_queue:
            num_trials += 1
            if num_trials > self.trials_limit:
                raise ValueError(
