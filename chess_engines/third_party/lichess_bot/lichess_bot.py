...
def logging_listener_proc(
    queue: LOGGING_QUEUE_TYPE, level: int, log_filename: Optional[str]
) -> None:
...
def game_logging_configurer(
    queue: Union[CONTROL_QUEUE_TYPE, LOGGING_QUEUE_TYPE], level: int
) -> None:
...
def start(
    ...
    correspondence_pinger = multiprocessing.Process(
        ...
        args=(control_queue, config.correspondence.checkin_period),
    )
    ...
    logging_listener = multiprocessing.Process(
        target=logging_listener_proc, args=(logging_queue, logging_level, log_filename)
    )
...
def lichess_bot_main(
    ...
    startup_correspondence_games = [
        game["gameId"] for game in all_games if game["speed"] == "correspondence"
    ]
    active_games = set(
        game["gameId"]
...
def check_in_on_correspondence_games(
    pool: POOL_TYPE,
    event: EVENT_TYPE,
    correspondence_queue: CORRESPONDENCE_QUEUE_TYPE,
    challenge_queue: MULTIPROCESSING_LIST_TYPE,
...
def check_online_status(
    li: lichess.Lichess, user_profile: USER_PROFILE_TYPE, last_check_online_time: Timer
) -> None:
    global restart
...
def play_game(
    ...
    with engine_wrapper.create_engine(config) as engine:
        ...
        keyword_map: DefaultDict[str, str] = defaultdict(
            str, me=game.me.name, opponent=game.opponent.name
        )
        hello = get_greeting("hello", config.greeting, keyword_map)
        goodbye = get_greeting("goodbye", config.greeting, keyword_map)
        hello_spectators = get_greeting(
            "hello_spectators", config.greeting, keyword_map
        )
        goodbye_spectators = get_greeting(
            "goodbye_spectators", config.greeting, keyword_map
        )

        disconnect_time = (
            correspondence_disconnect_time if not game.state.get("moves") else 0
        )
        prior_game = None
        board = chess.Board()
        ...
        while not terminated:
            ...
            try:
                ...
                if u_type == "chatLine":
                    ...
                    board, game, prior_game, li, is_correspondence
                ):
                    break
            except (
                HTTPError,
...
def get_greeting(
    greeting: str, greeting_cfg: Configuration, keyword_map: DefaultDict[str, str]
) -> str:
...
def say_hello(
    conversation: Conversation, hello: str, hello_spectators: str, board: chess.Board
) -> None:
...
def next_update(lines: Iterator[bytes]) -> GAME_EVENT_TYPE:
    ...
    upd: GAME_EVENT_TYPE = (
        json.loads(binary_chunk.decode("utf-8")) if binary_chunk else {}
    )
    if upd:
...
def is_engine_move(
    ...
    return game_changed(game, prior_game) and game.is_white == (
...
def print_pgn_game_record(
    ...
    lichess_game_record = (
        chess.pgn.read_game(io.StringIO(li.get_game_pgn(game.id))) or chess.pgn.Game()
    )
    try:
    ...
    lichess_node: Union[
        chess.pgn.Game, chess.pgn.ChildNode
    ] = lichess_game_record.game()
        ...
        if next_lichess_node:
            ...
            if current_node.comment != lichess_node.comment:
                current_node.comment = (
                    f"{current_node.comment} {lichess_node.comment}".strip()
                )

        commentary = engine.comment_for_board_index(index)
        pv_node = (
            current_node.parent.add_line(commentary["pv"])
            if "pv" in commentary
            else current_node
        )
...
def fill_missing_pgn_headers(game_record: chess.pgn.Game, game: model.Game) -> None:
    ...
    for header, game_value in local_headers.items():
        ...
        if (
            not record_value
            or record_value.startswith("?")
            or (header == "Result" and record_value == "*")
        ):
...
def check_python_version() -> None:
    ...
    python_deprecated_version = version_numeric(
        versioning_info["deprecated_python_version"]
    )
    ...
    upgrade_request = (
        f"You are currently running {version_str(this_python_version)}. "
