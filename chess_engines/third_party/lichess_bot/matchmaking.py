...
from chess_engines.third_party.lichess_bot.config import Configuration, DelayType
...
class Matchmaking:
    def __init__(
        ...
        self.last_challenge_created_delay = Timer(
            25
        )  # The challenge expires 20 seconds after creating it.
        ...
        self.matchmaking_cfg.list_of_bots_names_to_challenge = [
            x.split(":") for x in self.matchmaking_cfg.list_of_bots_names_to_challenge
        ]
        _bots = [x[0] for x in self.matchmaking_cfg.list_of_bots_names_to_challenge]
        _run_times = [
            int(x[1]) for x in self.matchmaking_cfg.list_of_bots_names_to_challenge
        ]
        _max_trials = [
            int(self.matchmaking_cfg.max_trial_number) for _ in range(len(_bots))
        ]
        _max_trials = [trial * run for trial, run in zip(_max_trials, _run_times)]
        self.white_list = defaultdict()
        for i in range(len(_bots)):
            d = {"run_times": _run_times[i], "max_trials": _max_trials[i]}
            self.white_list[_bots[i]] = d
        self.stop_after_whitelist_empty = (
            self.matchmaking_cfg.stop_after_whitelist_empty
        )
        self.delay_timers: DefaultDict[
            Union[str, Tuple[str, str, str, str]], Timer
        ] = defaultdict(Timer)
        ...
        if self.delay_type not in DelayType.__members__.values():
            raise ValueError(
                f"{self.delay_type} is not a valid value for {delay_option} parameter."
    ...
    def choose_opponent(self) -> Tuple[Optional[str], int, int, int, str, str]:
        ...
        if self.white_list:
            online_bots = list(filter(is_suitable_opponent_whitelist, online_bots))
        else:
            if not self.stop_after_whitelist_empty:
                online_bots = list(filter(is_suitable_opponent_random, online_bots))
            else:
                raise NoMoreBotsToChallenge("No more games to play.")

        def ready_for_challenge(bot: USER_PROFILE_TYPE) -> bool:
            return self.get_delay_timer(
                bot["username"], variant, game_type, mode
            ).is_expired()
        ...
        try:
            ...
            if bot_profile.get("blocking"):
                ...
                if bot["username"] in self.white_list:
                    self.white_list[bot["username"]]["max_trials"] -= 1
                    if self.white_list[bot["username"]]["max_trials"] == 0:
                        del self.white_list[bot["username"]]
                logger.info(
                    f"Removed {bot['username']} from white list as its profile is >>blocking<<."
                )
            else:
        ...
        # bot was successfully chosen, remove it from the white list and reduce its max_trials
        if bot_username in self.white_list:
            self.white_list[bot["username"]]["run_times"] -= 1
            self.white_list[bot["username"]]["max_trials"] -= 1
            if any(
                [
                    self.white_list[bot["username"]]["run_times"] == 0,
                    self.white_list[bot["username"]]["max_trials"] == 0,
                ]
            ):
                del self.white_list[bot["username"]]
    ...
    def challenge(
        self, active_games: Set[str], challenge_queue: MULTIPROCESSING_LIST_TYPE
    ) -> None:
        ...
        try:
            logger.info("Challenging a bot from the bot list")
    ...
    def declined_challenge(self, event: EVENT_TYPE) -> None:
        ...
        delay_timer = self.get_delay_timer(
            opponent.name, challenge.variant, challenge.speed, mode
        )
        ...
        if self.delay_type == DelayType.FINE:
            logger.info(
                f"Will not challenge {opponent} to a {mode} {challenge.speed} "
                f"{challenge.variant} game for {int(delay_timer.duration/3600)} {hours}."
            )
        else:
            logger.info(
                f"Will not challenge {opponent} for {int(delay_timer.duration/3600)} {hours}."
            )

    def get_delay_timer(
        self, opponent_name: str, variant: str, time_control: str, rated_mode: str
    ) -> Timer:
        if self.delay_type == DelayType.FINE:
