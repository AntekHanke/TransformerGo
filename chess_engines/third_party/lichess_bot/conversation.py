...
class Conversation:
    ...
    def react(self, line: ChatLine, game: model.Game) -> None:
        logger.info(
            f'*** {self.game.url()} [{line.room}] {line.username}: {line.text.encode("utf-8")!r}'
        )
    ...
    def command(self, line: ChatLine, game: model.Game, cmd: str) -> None:
        ...
        if cmd == "commands" or cmd == "help":
            self.send_reply(
                line,
                "Supported commands: !wait (wait a minute for my first move), !name, !howto, !eval, !queue",
            )
    ...
    def send_reply(self, line: ChatLine, reply: str) -> None:
        logger.info(
            f"*** {self.game.url()} [{line.room}] {self.game.username}: {reply}"
        )
    ...
    def send_message(self, room: str, message: str) -> None:
        if message:
            self.send_reply(
                ChatLine({"room": room, "username": "", "text": ""}), message
            )
