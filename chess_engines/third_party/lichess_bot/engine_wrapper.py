...
@contextmanager
def create_engine(
    engine_config: config.Configuration,
) -> Generator[EngineWrapper, None, None]:
    ...
    options = remove_managed_options(
        cfg.lookup(f"{engine_type}_options") or config.Configuration({})
    )
...
class EngineWrapper:
    def __init__(
        self, options: OPTIONS_TYPE, draw_or_resign: config.Configuration
    ) -> None:
        ...
        if isinstance(best_move, list) or best_move.move is None:
            ...
            if len(board.move_stack) < 2:
                best_move = choose_first_move(
                    self, board, game, draw_offered, best_move
                )
    ...
    def first_search(
        self, board: chess.Board, movetime: int, draw_offered: bool, root_moves: MOVE
    ) -> chess.engine.PlayResult:
    ...
    def offer_draw_or_resign(
        self, result: chess.engine.PlayResult, board: chess.Board
    ) -> chess.engine.PlayResult:
    ...
    def get_stats(self, for_chat: bool = False) -> List[str]:
        ...
        def to_readable_value(stat: str, info: MOVE_INFO_TYPE) -> str:
            readable: Dict[str, Callable[[Any], str]] = {
                "score": self.readable_score,
                "wdl": self.readable_wdl,
        ...
        if for_chat and "ponderpv" in info:
            bot_stats = [
                f"{to_readable_key(stat)}: {to_readable_value(stat, info)}"
                for stat in stats
            ...
            try:
                while (
                    len(" ".join(ponder_pv)) + len_bot_stats
                    > lichess.MAX_CHAT_MESSAGE_LEN
                ):
                    ponder_pv.pop()
...
class UCIEngine(EngineWrapper):
    def __init__(
        ...
        stderr: Optional[int],
        draw_or_resign: config.Configuration,
        **popen_args: str,
    ) -> None:
    ...
    def get_opponent_info(self, game: model.Game) -> None:
        ...
        if (
            name
            and isinstance(self.engine.protocol, chess.engine.UciProtocol)
            ...
            self.engine.configure(
                {"UCI_Opponent": f"{title} {rating} {player_type} {name}"}
            )
...
class XBoardEngine(EngineWrapper):
    def __init__(
        ...
        **popen_args: str,
        ...
        super().__init__(options, draw_or_resign)
        self.engine = chess.engine.SimpleEngine.popen_xboard(
            commands,
            timeout=10.0,
            debug=False,
            setpgrp=False,
    ...
    def get_opponent_info(self, game: model.Game) -> None:
        if (
            game.opponent.name
            and isinstance(self.engine.protocol, chess.engine.XBoardProtocol)
        ...
        if game.me.rating and game.opponent.rating:
            self.engine.protocol.send_line(
                f"rating {game.me.rating} {game.opponent.rating}"
            )
...
class FillerEngine:
    ...
    def __init__(self, main_engine: MinimalEngine, name: str = "") -> None:
        self.id: Dict[str, str] = {"name": name}
        self.name = name
...
def choose_move(
    ...
    logger.info(
        "Searching for wtime {wtime} btime {btime}".format_map(game.state)
        + f" for game {game.id}"
    )
...
def get_book_move(
    board: chess.Board, game: model.Game, polyglot_cfg: config.Configuration
) -> chess.engine.PlayResult:
...
def get_online_move(
    ...
    board: chess.Board,
    game: model.Game,
    online_moves_cfg: config.Configuration,
    draw_or_resign_cfg: config.Configuration,
) -> Union[chess.engine.PlayResult, List[chess.Move]]:
    ...
    if best_move is not None:
        ...
        comment = {
            "score": chess.engine.PovScore(
                chess.engine.Cp(wdl_to_score[wdl]), board.turn
            )
        }
    ...
    if (
        best_move is None
        and out_of_online_opening_book_moves[game.id] < max_out_of_book_moves
    ):
    ...
    if (
        out_of_online_opening_book_moves[game.id] == max_out_of_book_moves
        and used_opening_books
    ):
        logger.info(f"Will stop using online opening books for game {game.id}.")
    return chess.engine.PlayResult(None, None)


def get_chessdb_move(
    li: lichess.Lichess,
    board: chess.Board,
    game: model.Game,
    chessdb_cfg: config.Configuration,
    ...
    action = {"best": "querypv", "good": "querybest", "all": "query"}
    try:
        params = {"action": action[quality], "board": board.fen(), "json": 1}
        ...
        if data["status"] == "ok":
            if quality == "best":
                ...
                if depth >= chessdb_cfg.min_depth:
                    score = data["score"]
                    move = data["pv"][0]
                    comment["score"] = chess.engine.PovScore(
                        chess.engine.Cp(score), board.turn
                    )
                    comment["depth"] = data["depth"]
                    comment["pv"] = list(map(chess.Move.from_uci, data["pv"]))
                    logger.info(
                        f"Got move {move} from chessdb.cn (depth: {depth}, score: {score}) for game {game.id}"
                    )
            else:
                move = data["move"]
...
def get_lichess_cloud_move(
    li: lichess.Lichess,
    board: chess.Board,
    game: model.Game,
    lichess_cloud_cfg: config.Configuration,
) -> Tuple[Optional[str], Optional[chess.engine.InfoDict]]:
    wb = "w" if board.turn == chess.WHITE else "b"
    time_left = game.state[f"{wb}time"]
...
def get_online_egtb_move(
    li: lichess.Lichess,
    board: chess.Board,
    game: model.Game,
    online_egtb_cfg: config.Configuration,
) -> Tuple[Union[str, List[str], None], int]:
...
def get_egtb_move(
    board: chess.Board,
    game: model.Game,
    lichess_bot_tbs: config.Configuration,
    draw_or_resign_cfg: config.Configuration,
) -> Union[chess.engine.PlayResult, List[chess.Move]]:
    ...
    if best_move:
        ...
        if isinstance(best_move, chess.Move):
            return chess.engine.PlayResult(
                best_move, None, comment, draw_offered=offer_draw, resigned=resign
            )
        return best_move
    return chess.engine.PlayResult(None, None)


def get_lichess_egtb_move(
    li: lichess.Lichess,
    game: model.Game,
    board: chess.Board,
    quality: str,
    ...
    if pieces <= max_pieces:
        data = li.online_book_get(
            f"http://tablebase.lichess.ovh/{variant}", params={"fen": board.fen()}
        )
        if quality == "best":
            ...
            if dtm:
                dtm *= -1
            logger.info(
                f"Got move {move} from tablebase.lichess.ovh (wdl: {wdl}, dtz: {dtz}, dtm: {dtm}) for game {game.id}"
            )
            ...
            if len(possible_moves) > 1:
                ...
                wdl = best_wdl * -1
                logger.info(
                    f"Suggesting moves from tablebase.lichess.ovh (wdl: {wdl}) for game {game.id}"
                )
            else:
...
def get_chessdb_egtb_move(
    li: lichess.Lichess, game: model.Game, board: chess.Board, quality: str
) -> Tuple[Union[str, List[str], None], int]:
    def score_to_wdl(score: int) -> int:
        return piecewise_function([(-20001, 2), (-1, -1), (0, 0), (20000, 1)], 2, score)
    ...
    if data["status"] == "ok":
        if quality == "best":
            ...
            logger.info(
                f"Got move {move} from chessdb.cn (wdl: {wdl}, dtz: {dtz}) for game {game.id}"
            )
        elif quality == "suggest":
            ...
            if len(possible_moves) > 1:
                ...
                move = [move["uci"] for move in possible_moves]
                logger.info(
                    f"Suggesting moves from from chessdb.cn (wdl: {wdl}) for game {game.id}"
                )
            else:
                best_move = possible_moves[0]
...
def get_syzygy(
    board: chess.Board, game: model.Game, syzygy_cfg: config.Configuration
) -> Tuple[Union[chess.Move, List[chess.Move], None], int]:
    if (
        not syzygy_cfg.enabled
    ...
    with chess.syzygy.open_tablebase(syzygy_cfg.paths[0]) as tablebase:
        ...
        try:
            ...
            good_moves = [
                (move, dtz)
                for move, dtz in moves.items()
                if dtz_to_wdl(dtz) == best_wdl
            ]
            if move_quality == "good":
                move, dtz = random.choice(good_moves)
                logger.info(
                    f"Got move {move.uci()} from syzygy (wdl: {best_wdl}, dtz: {dtz}) for game {game.id}"
                )
                return move, best_wdl
            elif move_quality == "suggest" and len(good_moves) > 1:
                move = [chess_move for chess_move, dtz in good_moves]
                logger.info(
                    f"Suggesting moves from syzygy (wdl: {best_wdl}) for game {game.id}"
                )
                return move, best_wdl
            else:
                best_dtz = min([dtz for chess_move, dtz in good_moves])
                best_moves = [
                    chess_move for chess_move, dtz in good_moves if dtz == best_dtz
                ]
                move = random.choice(best_moves)
                logger.info(
                    f"Got move {move.uci()} from syzygy (wdl: {best_wdl}, dtz: {best_dtz}) for game {game.id}"
                )
                return move, best_wdl
        except KeyError:
            # Attempt to only get the WDL score. It returns a move of quality="good", even if quality is set to "best".
            try:
                moves = score_syzygy_moves(
                    board, lambda tablebase, b: -tablebase.probe_wdl(b), tablebase
                )
                best_wdl = max(moves.values())
                good_chess_moves = [
                    chess_move for chess_move, wdl in moves.items() if wdl == best_wdl
                ]
                logger.debug(
                ...
                if move_quality == "suggest" and len(good_chess_moves) > 1:
                    ...
                    logger.info(
                        f"Got move {move.uci()} from syzygy (wdl: {best_wdl}) for game {game.id}"
                    )
                return move, best_wdl
            except KeyError:
                return None, -3
...
def get_gaviota(
    board: chess.Board, game: model.Game, gaviota_cfg: config.Configuration
) -> Tuple[Union[chess.Move, List[chess.Move], None], int]:
    if (
        not gaviota_cfg.enabled
    ...
    with chess.gaviota.open_tablebase(gaviota_cfg.paths[0]) as tablebase:
        ...
        try:
            ...
            good_moves = [
                (move, dtm)
                for move, dtm in moves.items()
                if dtm_to_gaviota_wdl(dtm) == best_wdl
            ]
            best_dtm = min([dtm for move, dtm in good_moves])

            pseudo_wdl = dtm_to_wdl(best_dtm, min_dtm_to_consider_as_wdl_1)
            if move_quality == "good":
                best_moves = good_enough_gaviota_moves(
                    good_moves, best_dtm, min_dtm_to_consider_as_wdl_1
                )
                move, dtm = random.choice(best_moves)
                logger.info(
                    f"Got move {move.uci()} from gaviota (pseudo wdl: {pseudo_wdl}, dtm: {dtm}) for game {game.id}"
                )
                ...
                if len(best_moves) > 1:
                    ...
                    logger.info(
                        f"Suggesting moves from gaviota (pseudo wdl: {pseudo_wdl}) for game {game.id}"
                    )
                else:
                    move, dtm = random.choice(best_moves)
                    logger.info(
                        f"Got move {move.uci()} from gaviota (pseudo wdl: {pseudo_wdl}, dtm: {dtm})"
                        f" for game {game.id}"
                    )
            else:
                # There can be multiple moves with the same dtm.
                best_moves = [
                    (move, dtm) for move, dtm in good_moves if dtm == best_dtm
                ]
                move, dtm = random.choice(best_moves)
                logger.info(
                    f"Got move {move.uci()} from gaviota (pseudo wdl: {pseudo_wdl}, dtm: {dtm}) for game {game.id}"
...
def dtm_to_wdl(dtm: int, min_dtm_to_consider_as_wdl_1: int) -> int:
    ...
    return piecewise_function(
        [(-100, -1), (-1, -2), (0, 0), (min_dtm_to_consider_as_wdl_1 - 1, 2)], 1, dtm
    )


def good_enough_gaviota_moves(
    good_moves: List[Tuple[chess.Move, int]],
    best_dtm: int,
    min_dtm_to_consider_as_wdl_1: int,
    ...
    if best_dtm < 100:
        ...
        return [
            (move, dtm)
            for move, dtm in good_moves
            if dtm < min_dtm_to_consider_as_wdl_1
        ]
    elif best_dtm <= -min_dtm_to_consider_as_wdl_1:
        # If a move had wdl=-2 and dtz=-98, but halfmove_clock is 4 then the real wdl=-1 and dtz=-102, so we
...
def piecewise_function(
    range_definitions: List[Tuple[int, int]], last_value: int, position: int
) -> int:
...
def score_gaviota_moves(
    board: chess.Board,
    scorer: Callable[
        [
            Union[chess.gaviota.NativeTablebase, chess.gaviota.PythonTablebase],
            chess.Board,
        ],
        int,
    ],
