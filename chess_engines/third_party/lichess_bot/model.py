...
class Challenge:
    ...
    def is_supported_recent(
        ...
        recent_bot_challenges[self.challenger.name] = [
            timer
            for timer in recent_bot_challenges[self.challenger.name]
...
class Game:
    def __init__(
        self, json: Dict[str, Any], username: str, base_url: str, abort_time: int
    ) -> None:
        ...
        self.game_start = datetime.datetime.fromtimestamp(
            json["createdAt"] / 1000, tz=datetime.timezone.utc
        )
        self.abort_time = Timer(abort_time)
        self.terminate_time = Timer(
            (self.clock_initial + self.clock_increment) / 1000 + abort_time + 60
        )
