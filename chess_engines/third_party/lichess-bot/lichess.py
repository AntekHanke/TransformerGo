...
class Lichess:
    ...
    def set_rate_limit_delay(self, path_template: str, delay_time: int) -> None:
        logger.warning(
            f"Endpoint {path_template} is rate limited. Waiting {delay_time} seconds until next request."
        )
    ...
    def challenge(
        self, username: str, payload: REQUESTS_PAYLOAD_TYPE
    ) -> JSON_REPLY_TYPE:
        return self.api_post(
    ...
    def online_book_get(
        ...
        def online_book_get() -> JSON_REPLY_TYPE:
            json_response: JSON_REPLY_TYPE = self.other_session.get(
                path, timeout=2, params=params
            ).json()
            return json_response
